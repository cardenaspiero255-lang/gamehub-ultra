...
object PeripheralDiagnosticsProvider {
    fun get(context: Context): PeripheralDiagnostics {
        ...
        val externalAudio = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val audioManager = context.getSystemService(AudioManager::class.java)
            audioManager?.getDevices(AudioManager.GET_DEVICES_OUTPUTS)
                ?.count(::isExternalAudioDevice)
                ?: 0
        } else {
            0
        }
        ...
        return PeripheralDiagnostics(
            ...
            mouseCount = mice,
            externalAudioCount = externalAudio
        )
    }
    ...
    private fun isExternalAudioDevice(device: AudioDeviceInfo): Boolean =
        when (device.type) {
            ...
            AudioDeviceInfo.TYPE_MULTICHANNEL_GROUP -> false
