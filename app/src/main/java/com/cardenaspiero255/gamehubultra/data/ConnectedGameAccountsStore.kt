...
class ConnectedGameAccountsStore(context: Context) {
    ...
    suspend fun add(platform: GamePlatform, displayName: String, publicId: String): ConnectedGameAccount {
    ...
    private fun encode(account: ConnectedGameAccount): String =
        listOf(account.id, account.platform.name, account.displayName, account.publicId)
            .joinToString("|") {
